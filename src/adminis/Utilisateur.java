...
public class Utilisateur extends JFrame implements ActionListener {
	...
	public Utilisateur(){
		...
		add(pn);
		//titre
		lbtitre=new JLabel("Enregistrement des utilisateurs");
		...
		String rq="select * from utilisateur";
		try{
			st=cn.connecion().createStatement();
			rst=st.executeQuery(rq);
			while(rst.next()){
				df.addRow(new Object[]{
						rst.getString("idutilisateur"),rst.getString("nomUtilisateur")
				});
	...
	public void actionPerformed(ActionEvent e) {
		...
		String qr="insert into utilisateur values('"+a+"','"+b+"')";
		try{
			st=cn.connecion().createStatement();
		...
		if(e.getSource()==bt2){
			String a;
			a=tf1.getText();
			String qr="delete from utilisateur where idutilisateur='"+a+"'";
			try{
				st=cn.connecion().createStatement();
      ...
      if(e.getSource()==bt4){
    	  String a;
			a=tf1.getText();
			String qr="select * from utilisateur where idutilisateur='"+a+"'";
			try{
				st=cn.connecion().createStatement();
				rst=st.executeQuery(qr);
				if(rst.next()){
					tf2.setText(rst.getString("nomUtilisateur"));
				}
				else
		...
		if(e.getSource()==bt6){
			String a,b;
			a=tf1.getText();b=tf2.getText();
			String qr="update utilisateur set nomUtilisateur='"+b+"' where idutilisateur='"+a+"'";
			try{
				st=cn.connecion().createStatement();
