...
public class Requete extends JFrame implements ActionListener {
	...
	public Requete(){
		this.setTitle("biblio_esp(from DGI)");
		...
		add(pn);
		//lb
		lb1=new JLabel("Livres emprunt�s");
...
try{
	...
	jcb1.addItem(rst.getString("titre"));
		...
		//ajout des titres des livres au combo livres
		String kk6="select nomUtilisateur from utilisateur";
try{
	st=cn.connecion().createStatement();
	rst=st.executeQuery(kk6);
	while(rst.next()){
	jcb2.addItem(rst.getString("nomUtilisateur"));
	...
	//jcb3	
		...
		try{
			...
			jcb3.addItem(rst.getString("titre"));
				...
				pn.add(jcb3);
		//bt
		bt2=new JButton("V�rifier");
		bt2.setBounds(210,330,100,25);
		bt2.addActionListener(this);
		pn.add(bt2);
		//bt
				bt3=new JButton("Utilisateur");
		...
		tb.setModel(df);
		pn.add(js);
		String qr="select * from utilisateur inner join livre on utilisateur.idutilisateur=livre.idutilisateur";
		try{
			st=cn.connecion().createStatement();
			rst=st.executeQuery(qr);
			while(rst.next()){
			df.addRow(new Object[]{
					rst.getString("idlivre"),rst.getString("titre"),rst.getString("nomUtilisateur"),rst.getString("idutilisateur")
			});	
			}
		}
		catch(SQLException ex){
	...
	public void actionPerformed(ActionEvent e) {
		...
		String qr="select * from pret where titre='"+b+"' or nomUtilisateur='"+c+"'";
		...
		try{
			...
			df2.addRow(new Object[]{
					rst.getString("idlivre"),rst.getString("titre"),rst.getString("nomUtilisateur")
			});	
			}
		}
		catch(SQLException ex){
