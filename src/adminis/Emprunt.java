...
public class Emprunt extends JFrame implements ActionListener {
	...
	JButton jb,jb2,bt3,blivre,butilisateur;
	public Emprunt(){
...
try{
	...
	jcb2.addItem(rst.getString("titre"));
		...
		//bouton livre
				...
				pn.add(blivre);
	 //bouton utlisateur
				butilisateur=new JButton("Utilisateur");
				butilisateur.setBounds(260,420,110,30);
				butilisateur.addActionListener(this);
				pn.add(butilisateur);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==jb){
			...
			String qr="update livre set disponible='NON',idutilisateur='"+a+"' where titre='"+b+"'";
			try{
				st=cn.connecion().createStatement();
		...
		if(e.getSource()==jb2){
			...
			String qr="update livre set disponible='OUI',idutilisateur=NULL where idutilisateur='"+a+"' and titre='"+b+"'";
			try{
				st=cn.connecion().createStatement();
				if(JOptionPane.showConfirmDialog(this,"ajouter?",null,JOptionPane.OK_CANCEL_OPTION)==JOptionPane.OK_OPTION){
					st.executeUpdate(qr);
